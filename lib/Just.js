...
Just.require = function (pkg) {
    ...
    // Perhaps defined elsewhere ...
    if(Just.findPackage(pkg))
      return Just.findPackage(pkg);
...
Just.findPackage = function(pkg) {
    var ns = pkg.split('.');
    for(var i = 0, curNs = Just.globalScope; i < ns.length; i++) {
        if(!curNs || !(ns[i] in curNs))
          return false;
        curNs = curNs[ns[i]];
    }
    return curNs;
...
Just._makeNamespace = function(js, pkg) {
    ...
    eval(js);
    var classdef = Just.findPackage(pkg);
...
Just.Request.prototype = {
    ...
    // This doesn't work locally for either IE or Chrome.
    getText: function (url) {
        try {
            ...
            Just._handleError("Couldn't load '"+url+"': " + e.message);
            return null;
        };
